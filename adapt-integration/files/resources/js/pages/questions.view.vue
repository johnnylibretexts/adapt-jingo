...
export default {
  ...
  methods: {
    ...
    submittedPronunciation ({ response, upload }) {
      // Mirrors submittedAudioUpload (questions.view.vue:4659). Uses the upload
      // response for the file fields (submission_file_url, date_submitted) and the
      // /api/jwt/process-answer-jwt gradeback response for type/message/completion.
      // Submission::store does NOT return score/date_submitted (unlike the open-ended
      // submission-texts/submission-audios controllers), so updateTotalScore() refreshes
      // the total after Score::updateAssignmentScore runs server-side.
      const data = response
      this.openEndedSubmissionDataType = (data.type === 'success' || data.type === 'info') ? 'success' : 'danger'
      this.submissionDataMessage = data.message
      let modalToShow
      if (this.openEndedSubmissionDataType !== 'success') {
        modalToShow = 'modal-thumbs-down'
        ...
        this.cacheKey++
        modalToShow = data.completed_all_assignment_questions
          ? 'modal-completed-assignment'
          : 'modal-submission-accepted'
      }
      this.$bvModal.show(modalToShow)
      if (data.type === 'success' || data.type === 'info') {
        this.questions[this.currentPage - 1].date_submitted = upload.date_submitted
        this.questions[this.currentPage - 1].submission_file_url = upload.submission_file_url
        this.questions[this.currentPage - 1].submission_file_exists = true
        this.updateTotalScore()
      }
    },
