...
class Submission extends Model
{
    ...
    /**
     * Pronunciation proportion correct for Submission::store's pronunciation case.
     *
     * Hybrid grading: completion credit by default and for every scorer failure
     * (engine/provider/audio); score-based only when grading=='score' AND the
     * service returned a 'scored' status with a usable overall. The gradebook
     * never punishes the student for a scorer failure.
     *
     * Integrity: the exercise_id/language the service scored (echoed in the
     * answerJWT score) must match this question's stored pronunciation_data.
     * A mismatch means the attempt was scored against a different record than
     * the question owns → reject (no credit), mirroring the unverified-callback
     * rule. The audio is already in S3, so the student's work is preserved.
     *
     * @param object $score the answerJWT `score` payload (cast from array)
     * @param array $pronData decoded questions.pronunciation_data
     * @return float 0.0–1.0
     * @throws Exception on exercise_id/language mismatch
     */
    public function computePronunciationProportion(object $score, array $pronData): float
    {
        // Never punish the student for a scorer failure: any non-'scored' status
        // (engine_unavailable, provider_miss, audio_unreachable, unscoreable, …)
        // always yields completion credit, regardless of grading mode or integrity.
        if (($score->status ?? '') !== 'scored') {
            return 1.0;
        }
        if (($pronData['exercise_id'] ?? null) !== ($score->exercise_id ?? null)
            || ($pronData['language'] ?? null) !== ($score->language ?? null)) {
            throw new Exception(
                "Pronunciation submission exercise_id/language mismatch — flagged for review."
            );
        }
        $grading = $pronData['grading'] ?? 'completion';
        if ($grading === 'score' && ($score->status ?? '') === 'scored' && isset($score->overall)) {
            return floatval($score->overall) / 100.0;
        }
        return 1.0; // completion credit: default + every scorer failure
    }
                   ...
                   AssignmentSyncQuestion $assignmentSyncQuestion): array
    {
        ...
        switch ($data['technology']) {
            ...
            case('qti'):
                ...
                break;
            case('pronunciation'):
                $submission = $data['submission'];              // the answerJWT object
                $scoreObj = (object) $submission->score;        // score is an array; cast to object
                $question = DB::table('questions')
                    ->where('id', $data['question_id'])->first();
                if (!$question) {
                    $response['message'] = "{$data['question_id']} does not exist in the database.";
                ...
                $pronData = $question->pronunciation_data
                    ? json_decode($question->pronunciation_data, true) : [];
                $proportion_correct = $this->computePronunciationProportion($scoreObj, $pronData);
                $data['score'] = $assignment->scoring_type === 'p'
                    ? floatval($assignment_question->points) * $proportion_correct
                    : $this->computeScoreForCompletion($assignment_question);
                $data['submission'] = json_encode($data['submission'], JSON_UNESCAPED_SLASHES);
