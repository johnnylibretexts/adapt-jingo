...
//limit search in the database so that the library and page_id exist (may be separate)
                                           ...
                                           array       $additional_custom_claims = [])
    {
        ...
        switch ($question->technology) {
            ...
            case('webwork'):
                ...
                */
                if (in_array($webwork_domain, ['https://wwrenderer.libretexts.org', 'https://wwrenderer-staging.libretexts.org', config('myconfig.webwork_renderer_url')])) {
 ...
 <iframe id="embed1" style="position:absolute;z-index:1" frameborder="0" src="https://imathas.libretexts.org/imathas/adapt/embedq2.php?frame_id=embed1"></iframe>
            ...
            case('h5p'):
                ...
                break;
            case('pronunciation'):
                // No iframe; the Vue PronunciationQuestion component renders the
                // recorder (see resources/js/components/questions/PronunciationQuestion.vue).
                // exercise_id + language travel to the renderer via the question's
                // pronunciation_data column and are trusted at grade time in
                // Submission::store (computePronunciationProportion mismatch check).
                // The problemJWT carries only the standard adapt claims; the service
                // echoes it opaquely and never decrypts it (no cross-language JWE).
                $custom_claims['pronunciation'] = [];
                $problemJWT = $this->createProblemJWT($JWE, $custom_claims, 'webwork');
                break;
            case('qti'):
            case('text'):
...
//not checking the double issue for questions with images
    ...
    function getWebworkCodeFromFilePath($file_path)
    {
        ...
        $endpoint = $this->getWebworkDomain() . "/render-api/tap";
    ...
    function getWebworkHtmlFromCode($webwork_code)
    {
        ...
        $endpoint = $this->getWebworkDomain() . "/render-api";
        return $this->curlPost($endpoint, $data);
    ...
    {
        return config('myconfig.webwork_renderer_endpoint');
    ...
    {
        return rtrim(config('myconfig.webwork_renderer_url'), '/');
